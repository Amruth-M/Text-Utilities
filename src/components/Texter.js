...
<button disabled={Text.length===0} className="btn btn-primary  " onClick={clearClick}>Clear</button>
...
{/* <button className="btn btn-primary mx-2" onClick={handleBold}>Bold Case</button> */}
    ...
    <div className="container" style={{color:props.mode==='dark'?'white':'black'}}>
      ...
      <p>{Text.length} Letters and {Text.split(/\s+/).filter((element)=>{return element.length!==0}).length} words </p>
      <p>We need {0.50*Text.split(/\s+/).filter((element)=>{return element.length!==0}).length} seonds to read</p>
